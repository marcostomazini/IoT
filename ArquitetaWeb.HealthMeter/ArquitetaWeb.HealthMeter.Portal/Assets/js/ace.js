...
jQuery(function($) {
    ...
    window['arqWeb'].click_event = $.fn.tap ? "tap" : "click";
});

jQuery(function($) {
	//arqWeb.click_event defined in ace-elements.js
	arqWeb.handle_side_menu(jQuery);

	arqWeb.enable_search_ahead(jQuery);	

	arqWeb.general_things(jQuery);//and settings

	arqWeb.widget_boxes(jQuery);
	arqWeb.widget_reload_handler(jQuery);//this is for demo only, you can remove and have your own function, please see examples/widget.html
...
arqWeb.handle_side_menu = function($) {
	$('#menu-toggler').on(arqWeb.click_event, function() {
	...
	$('#sidebar-collapse').on(arqWeb.click_event, function(){
		$minimized = $('#sidebar').hasClass('menu-min');
		arqWeb.settings.sidebar_collapsed(!$minimized);//@ ace-extra.js
	});

	var touch = "ontouchend" in document;
	//opening submenu
	$('.nav-list').on(arqWeb.click_event, function(e){
		...
		if(! link_element.hasClass('dropdown-toggle') ) {//it doesn't have a submenu return
			...
			if($minimized && arqWeb.click_event == "tap" &&
...
arqWeb.general_things = function($) {
 ...
 $('#ace-settings-btn').on(arqWeb.click_event, function(){
 ...
 $('#ace-settings-navbar').on('click', function(){
	arqWeb.settings.navbar_fixed(this.checked);//@ ace-extra.js
 }).each(function(){this.checked = arqWeb.settings.is('navbar', 'fixed')})

 $('#ace-settings-sidebar').on('click', function(){
	arqWeb.settings.sidebar_fixed(this.checked);//@ ace-extra.js
 }).each(function(){this.checked = arqWeb.settings.is('sidebar', 'fixed')})

 $('#ace-settings-breadcrumbs').on('click', function(){
	arqWeb.settings.breadcrumbs_fixed(this.checked);//@ ace-extra.js
 }).each(function(){this.checked = arqWeb.settings.is('breadcrumbs', 'fixed')})

 $('#ace-settings-add-container').on('click', function(){
	arqWeb.settings.main_container_fixed(this.checked);//@ ace-extra.js
 }).each(function(){this.checked = arqWeb.settings.is('main-container', 'fixed')})
...
//     arqWeb.settings.main_rtl(this.checked);//@ ace-extra.js
//}).each(function () { this.checked = arqWeb.settings.is('main_rtl', 'left') })


 $('#btn-scroll-up').on(arqWeb.click_event, function(){
  ...
  $('#skin-colorpicker').on('change', function(){
	...
	arqWeb.settings.set_skin(0);//@ ace-extra.js
	if(skin_class != 'default') body.addClass(skin_class);

	if(skin_class == 'skin-1') {
	    $('.ace-nav > li.grey').addClass('dark');
	    arqWeb.settings.set_skin(1);//@ ace-extra.js
	}
	else {
	...
	if(skin_class == 'skin-2') {
		...
		arqWeb.settings.set_skin(2);//@ ace-extra.js
	}
	else {
	...
	if(skin_class == 'skin-3') {
	    ...
	    arqWeb.settings.set_skin(3);//@ ace-extra.js
	} else {
...
arqWeb.widget_boxes = function($) {
	...
	$(document).on('click.arqWeb.widget', '[data-action]', function (ev) {
		ev.preventDefault();
		...
		if($action == 'collapse') {
			...
			var event
			$box.trigger(event = $.Event(event_name+'.arqWeb.widget'))
			...
			if( event_name == 'show' ) {
				...
				$body.slideUp(0 , function(){$body.slideDown(expandSpeed, function(){$box.trigger(event = $.Event(event_complete_name+'.arqWeb.widget'))})});
			}
			else {
				if($icon) $icon.addClass($icon_down).removeClass($icon_up);
				$body.slideUp(collapseSpeed, function(){$box.addClass('collapsed');$box.trigger(event = $.Event(event_complete_name+'.arqWeb.widget'))});
			}
			...
			var event
			$box.trigger(event = $.Event('close.arqWeb.widget'))
			if (event.isDefaultPrevented()) return
			
			var closeSpeed = parseInt($this.data('close-speed')) || 300;
			$box.hide(closeSpeed , function(){$box.trigger(event = $.Event('closed.arqWeb.widget'));$box.remove();});
		}
		else if($action == 'reload') {
			var event
			$box.trigger(event = $.Event('reload.arqWeb.widget'))
			if (event.isDefaultPrevented()) return
			...
			$box.one('reloaded.arqWeb.widget', function() {
			...
			var event = $.Event('settings.arqWeb.widget')
			$box.trigger(event)
		}
...
arqWeb.widget_reload_handler = function($) {
	//***default action for reload in this demo
	//you should remove this and add your own handler for each specific .widget-box
	//when data is finished loading or processing is done you can call $box.trigger('reloaded.arqWeb.widget')
	$(document).on('reload.arqWeb.widget', '.widget-box', function (ev) {
		var $box = $(this);
		//trigger the reloaded event after 1-2 seconds
		setTimeout(function() {
			$box.trigger('reloaded.arqWeb.widget');
	...
	/**
	$('#my-widget-box').on('reload.arqWeb.widget', function(){
		//load new data 
		//when finished trigger "reloaded"
		$(this).trigger('reloaded.arqWeb.widget');
	});	
	*/
}
...
arqWeb.enable_search_ahead = function($) {
	arqWeb.variable_US_STATES = ["Alabama","Alaska","Arizona","Arkansas","California","Colorado","Connecticut","Delaware","Florida","Georgia","Hawaii","Idaho","Illinois","Indiana","Iowa","Kansas","Kentucky","Louisiana","Maine","Maryland","Massachusetts","Michigan","Minnesota","Mississippi","Missouri","Montana","Nebraska","Nevada","New Hampshire","New Jersey","New Mexico","New York","North Dakota","North Carolina","Ohio","Oklahoma","Oregon","Pennsylvania","Rhode Island","South Carolina","South Dakota","Tennessee","Texas","Utah","Vermont","Virginia","Washington","West Virginia","Wisconsin","Wyoming"]
	
	try {
		$('#nav-search-input').typeahead({
			source: arqWeb.variable_US_STATES,
...
arqWeb.switch_direction = function($) {
	var $body = $(document.body);
	$body
	.toggleClass('rtl')
