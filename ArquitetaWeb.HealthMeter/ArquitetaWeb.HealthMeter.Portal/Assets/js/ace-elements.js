...
(function($ , undefined) {
	...
	Ace_File_Input.prototype.apply_settings = function() {
		...
		if(remove_btn) this.$label.next('a').on(arqWeb.click_event, function(){
...
(function($ , undefined) {
	$.fn.ace_wizard = function(options) {
		...
		this.each(function() {
			...
			$wizard.$prevBtn = buttons.find('.btn-prev').eq(0).on(arqWeb.click_event,  function(){
				$this.wizard('previous');
			}).attr('disabled', 'disabled');
			$wizard.$nextBtn = buttons.find('.btn-next').eq(0).on(arqWeb.click_event,  function(){
				$this.wizard('next');
			}).removeAttr('disabled');
...
(function($ , undefined) {
	$.fn.ace_colorpicker = function(options) {
		...
		this.each(function() {
			...
			.next().find('.dropdown-menu').on(arqWeb.click_event, function(e) {
				var a = $(e.target);
...
(function($ , undefined) {
	$.fn.ace_wysiwyg = function($options , undefined) {
		...
		this.each(function() {
			...
			if(options.toolbar_place) toolbar = options.toolbar_plarqWeb.call(this, toolbar);
			...
			toolbar.find('.dropdown-menu input:not([type=file])').on(arqWeb.click_event, function() {return false})
			...
			toolbar.find('input[type=file]').prev().on(arqWeb.click_event, function (e) { 
				$(this).next().click();
			});
